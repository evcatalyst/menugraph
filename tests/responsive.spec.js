...
test("ingredient navigator renders CWA visual timeline without relying on public image pixels", async ({ page }) => {
  ...
  await expect(page.locator(".cwa-product-chip")).toHaveCount(100);
  ...
  await expect(page.locator("#source-family-coverage-summary")).toContainText("107/120 products represented");
  ...
  await expect(page.locator(".source-family-tab.is-selected")).toContainText("Official Current Labels");
  await expect(page.locator(".cwa-product-chip")).toHaveCount(100);
  await expect(page.locator("#source-family-filter-status")).toContainText("100 products");
  ...
  await expect(page.locator("#source-family-filter-status")).toContainText("of 100 products");
  await expect(page.locator(".cwa-product-chip")).not.toHaveCount(100);
  await page.locator("#source-family-filter-clear").click();
  await page.locator("#source-family-search").fill("sodium alginate");
  await expect(page.locator("#source-family-filter-status")).toContainText("1 of 100 products");
  await expect(page.locator(".cwa-product-chip")).toHaveCount(1);
  ...
  await expect(page.locator(".cwa-product-chip")).toHaveCount(100);
  await expect(page.locator("#source-family-position")).toContainText("1 / 100");
  await expect(page.locator("#source-family-prev")).toBeDisabled();
  await expect(page.locator("#source-family-next")).toBeEnabled();
  await page.locator("#source-family-next").click();
  await expect(page.locator("#source-family-position")).toContainText("2 / 100");
  ...
  await expect(page.locator("#source-family-filter-status")).toContainText("0 of 100 products");
