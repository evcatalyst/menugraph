...
test("ingredient navigator renders CWA visual timeline without relying on public image pixels", async ({ page }) => {
  ...
  await expect(page.locator(".cwa-product-chip")).toHaveCount(103);
  ...
  await expect(page.locator("#source-family-coverage-summary")).toContainText("116/120 products represented");
  ...
  await expect(page.locator(".source-family-tab.is-selected")).toContainText("Official Current Labels");
  await expect(page.locator(".cwa-product-chip")).toHaveCount(103);
  await expect(page.locator("#source-family-filter-status")).toContainText("103 products");
  ...
  await expect(page.locator("#source-family-filter-status")).toContainText("of 103 products");
  await expect(page.locator(".cwa-product-chip")).not.toHaveCount(103);
  await page.locator("#source-family-filter-clear").click();
  await page.locator("#source-family-search").fill("sodium alginate");
  await expect(page.locator("#source-family-filter-status")).toContainText("1 of 103 products");
  await expect(page.locator(".cwa-product-chip")).toHaveCount(1);
  ...
  await expect(page.locator(".cwa-product-chip")).toHaveCount(103);
  await expect(page.locator("#source-family-position")).toContainText("1 / 103");
  await expect(page.locator("#source-family-prev")).toBeDisabled();
  await expect(page.locator("#source-family-next")).toBeEnabled();
  await page.locator("#source-family-next").click();
  await expect(page.locator("#source-family-position")).toContainText("2 / 103");
  ...
  await expect(page.locator("#source-family-filter-status")).toContainText("0 of 103 products");
