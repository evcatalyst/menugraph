...
const OUTPUT_PATH = path.join(DATA_DIR, "enrichment", "external-sources", "milwaukee_historic_menus.json");
const BASE_URL = "https://content.mpl.org";
const DIGITAL_BASE = `${BASE_URL}/digital`;
const COLLECTION_ALIAS = "histmenu";
const SOURCE_ID = "milwaukee_historic_menus";
const SOURCE_KEY = "milwaukee";
const COLLECTION_TITLE = "Milwaukee Public Library Historic Menu Collection";
const VERSION = 1;

const CUISINE_PATTERNS = [
  ["chinese", /\bchinese\b|\bchinatown\b/i],
  ["delicatessen", /\bdelicatessen\b|\bdeli\b/i],
  ["french", /\bfrench\b/i],
  ["italian", /\bitalian\b|\bpizzas?\b|\bpastas?\b|\bristorante\b/i],
  ["seafood", /\bseafood\b|\bfish\b|\blake superior whitefish\b/i],
  ["steakhouse", /\bsteakhouse\b|\bsteaks?\b/i],
];

const DISH_HINT_PATTERNS = [
  ["alcoholic drinks", /\balcoholic drinks?\b|\bcocktails?\b/i],
  ["breakfast options", /\bbreakfast\b/i],
  ["dessert options", /\bdesserts?\b/i],
  ["dinner options", /\bdinners?\b/i],
  ["fish and seafood options", /\bseafood\b|\bfish\b/i],
  ["lunch options", /\blunch(?:es)?\b/i],
  ["pasta", /\bpastas?\b/i],
  ["pizza", /\bpizzas?\b/i],
...
function parseDateRange(value) {
  ...
  const ranges = [...text.matchAll(/\b((?:18|19|20)\d{2})\s*[-/]\s*((?:18|19|20)\d{2})\b/g)].map((match) => [Number(match[1]), Number(match[2])]);
  if (ranges.length) {
    const lowerYear = Math.min(...ranges.map((range) => Math.min(...range)));
    const upperYear = Math.max(...ranges.map((range) => Math.max(...range)));
  ...
  const decade = text.match(/\b((?:18|19|20)\d{2})s\b/i);
  if (decade) {
    const lowerYear = Number(decade[1]);
    const upperYear = lowerYear + 9;
    const pointYear = lowerYear + 5;
    return { dateText: text, year: null, lowerYear, upperYear, pointYear, decade: `${lowerYear}s`, confidence: "C" };
  }

  const year = text.match(/\b(18|19|20)\d{2}\b/);
  if (year) {
    const value = Number(year[0]);
    return { dateText: text, year: value, lowerYear: value, upperYear: value, pointYear: value, decade: `${Math.floor(value / 10) * 10}s`, confidence: "B" };
  }

  return { dateText: text, year: null, lowerYear: null, upperYear: null, pointYear: null, decade: "unknown", confidence: "X" };
}
...
function styleTagsFor(text) {
  ...
  const tags = [];
  if (/\btrifold\b/.test(normalized)) tags.push("trifold");
  if (/\bbrochure\b/.test(normalized)) tags.push("brochure");
  if (/\bplastic\b|\blaminated\b/.test(normalized)) tags.push("plastic cover");
  if (/\bmap\b/.test(normalized)) tags.push("map");
  if (/\bcover\b/.test(normalized)) tags.push("cover illustration");
  if (/\binsert\b/.test(normalized)) tags.push("insert");
  if (/\bcarry out\b|\btake out\b|\btakeout\b/.test(normalized)) tags.push("takeout");
  return [...new Set(tags)].sort();
}
...
function dishSegmentsFor({ description, cuisine }) {
  const text = [description, cuisine].map(cleanValue).join(" ");
  const segments = [];
...
function dishMentionFor(record, rawName, contextText) {
  ...
  return {
    id: stableId("milwaukeedish", [record.menuId, normalizedName]),
    ...
    ingredientTags: tags,
    extractionMethod: "milwaukee_metadata_keyword",
    confidence: 0.48,
    provenance: {
      sourceFile: "enrichment/external-sources/milwaukee_historic_menus.json",
...
function imageFeaturesFor(record, item) {
  ...
  return [
    {
      id: stableId("milwaukeeimage", [record.menuId, imageUri, iiifInfoUri, pages.length]),
      menuId: record.menuId,
      sourceId: SOURCE_ID,
      sourceKey: SOURCE_KEY,
      ...
      iiifInfoUri,
      modelName: "milwaukee_contentdm_metadata",
      confidence: 0.72,
      provenance: {
        sourceFile: "enrichment/external-sources/milwaukee_historic_menus.json",
        sourceRecordId: record.sourceRecordId,
...
function normalizeItem(item, searchItem = {}) {
  ...
  const title = firstField(item, ["title"]) || cleanValue(item.title || searchItem.title) || "Untitled Milwaukee menu";
  const restaurant = firstField(item, ["restau", "restaurant name"]) || title.replace(/^menu for\s+/i, "");
  const description = firstField(item, ["descri", "description"]);
  const cuisine = firstField(item, ["cuisin", "cuisine"]);
  const dateText = firstField(item, ["date"]) || firstField(item, ["decade"]);
  ...
  const city = firstField(item, ["city"]);
  const state = firstField(item, ["state"]) || "Wisconsin";
  const rightsStatement = firstField(item, ["rights"]);
  const rightsUri = firstField(item, ["righta", "rights uri"]);
  const contextText = [title, restaurant, description, cuisine, subjectTerms.join(" "), formatTerms.join(" ")].join(" ");
  ...
  const dishMentions = dishSegmentsFor({ description, cuisine }).map((segment) => dishMentionFor(baseRecord, segment, contextText)).filter(Boolean);
...
async function fetchJson(url, timeoutMs) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        "User-Agent": "MenuGraph bounded Milwaukee metadata connector; derived metadata only",
      },
    });
    if (!response.ok) throw new Error(`Milwaukee request returned HTTP ${response.status} for ${url}`);
...
async function buildMilwaukeeSource(options = {}) {
  const startedAt = new Date().toISOString();
  const maxRecords = Math.min(200, Math.max(1, options.limit || 100));
  ...
  const output = {
    ...
    sourceId: SOURCE_ID,
    sourceKey: SOURCE_KEY,
    collectionUrl: "https://content.mpl.org/digital/collection/histmenu",
...
async function main() {
  const output = await buildMilwaukeeSource(optionsFromArgs());
  console.log(
    [
      `Wrote ${output.summary.total.toLocaleString()} Milwaukee external menu records`,
