...
const SOURCE_HINTS = {
  ...
  tulane_louisiana_menu_collection: {
    ...
    routeBlocker: "Public collection is documented, but a stable bulk metadata/export route is not verified in the current connector set.",
  ...
  unlv_menus_art_of_dining: {
    ...
    routeStatus: "access_challenge_review",
    routeBlocker: "Public collection documentation is visible, but automated metadata access needs challenge/access review before scraper implementation.",
    recommendedNextAction: "source_route_review",
  },
...
function probeForSource(source, inputs = {}) {
  ...
  return {
    ...
    routeStatus: cleanValue(priorProbe?.routeStatus || hint.routeStatus || (sourceType === "commercial_modern_menu" ? "license_required" : "modeled_route")),
    routeBlocker: cleanValue(priorProbe?.routeBlocker || hint.routeBlocker),
    recommendedNextAction: cleanValue(priorProbe?.recommendedNextAction || hint.recommendedNextAction),
