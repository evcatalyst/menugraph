...
assert.strictEqual(sourceFamilyCoverage.totals.represented_products, 116, "source family coverage should include source-family enrichment");
assert.strictEqual(sourceFamilyCoverage.totals.missing_products, 4, "source family coverage should expose the remaining missing product queue");
assert.strictEqual(navigator.source_family_coverage?.totals?.missing_products, 4, "navigator should embed the remaining missing product queue");
...
["chipotle_chicken_burrito", "subway_italian_bmt", "subway_tuna_sub"].forEach((productId) => {
  assert(!sourceFamilyCoverage.missing_products.some((row) => row.product_id === productId), `${productId} should be represented by the official-current source-family lane`);
});
["capn_crunch_original", "chef_boyardee_beefaroni", "dr_pepper_original", "hostess_cupcakes", "hot_pockets_pepperoni_pizza", "little_debbie_swiss_rolls"].forEach((productId) => {
  assert(!sourceFamilyCoverage.missing_products.some((row) => row.product_id === productId), `${productId} should be represented by the label-database source-family lane`);
});
...
assert.strictEqual(labelDatabaseVisualIndex.source_family.id, "label-database-text-leads", "label database source-family id should be stable");
assert.strictEqual(labelDatabaseVisualIndex.totals.products, 8, "label database lane should add eight products in this slice");
assert.strictEqual(labelDatabaseVisualIndex.totals.rows, 8, "label database lane should add eight proof rows in this slice");
assert.strictEqual(labelDatabaseVisualIndex.totals.local_preview_available, 8, "label database lane should have local private proof panels");
...
const labelDatabaseTwinkiesRow = labelDatabaseVisualIndex.rows.find((row) => row.product_id === "twinkies");
assert(labelDatabaseTwinkiesRow, "label database lane should include Twinkies");
assert.strictEqual(labelDatabaseTwinkiesRow.vintage_label, "2010s", "Twinkies database proof should retain its source vintage");
assert(labelDatabaseTwinkiesRow.ingredient_items.includes("Partially Hydrogenated Vegetable Shortening (Contains One Or More: Soybean, Canola Or Palm Oil)"), "Twinkies ingredient list should expose partially hydrogenated shortening");
assert(labelDatabaseTwinkiesRow.ingredient_items.includes("Color Added (Yellow 5, Red 40)"), "Twinkies ingredient list should expose color additive row");
const labelDatabaseCapnCrunchRow = labelDatabaseVisualIndex.rows.find((row) => row.product_id === "capn_crunch_original");
assert(labelDatabaseCapnCrunchRow, "label database lane should include Cap'n Crunch");
assert(labelDatabaseCapnCrunchRow.ingredient_items.includes("Oat Flour"), "Cap'n Crunch ingredient list should expose oat flour");
assert(labelDatabaseCapnCrunchRow.ingredient_items.includes("BHT (to Preserve Freshness)"), "Cap'n Crunch ingredient list should expose BHT preservative");
const labelDatabaseDrPepperRow = labelDatabaseVisualIndex.rows.find((row) => row.product_id === "dr_pepper_original");
assert(labelDatabaseDrPepperRow, "label database lane should include Dr Pepper");
assert(labelDatabaseDrPepperRow.ingredient_items.includes("High Fructose Corn Syrup"), "Dr Pepper ingredient list should expose HFCS");
assert(labelDatabaseDrPepperRow.ingredient_items.includes("Sodium Benzoate (Preservative)"), "Dr Pepper ingredient list should expose sodium benzoate");
const labelDatabaseChefRow = labelDatabaseVisualIndex.rows.find((row) => row.product_id === "chef_boyardee_beefaroni");
assert(labelDatabaseChefRow, "label database lane should include Chef Boyardee Beefaroni");
assert(labelDatabaseChefRow.ingredient_items.includes("Textured Vegetable Protein (Soy Flour, Caramel Color)"), "Beefaroni ingredient list should expose textured vegetable protein");
const labelDatabaseHotPocketsRow = labelDatabaseVisualIndex.rows.find((row) => row.product_id === "hot_pockets_pepperoni_pizza");
assert(labelDatabaseHotPocketsRow, "label database lane should include Hot Pockets");
assert(labelDatabaseHotPocketsRow.ingredient_items.includes("Pepperoni (Pork, Beef, Salt, 2% or Less of Spices, Dextrose, Lactic Acid Starter Culture, Oleoresin of Paprika, Sodium Ascorbate, Flavoring, Sodium Nitrite, BHA, BHT, Citric Acid)"), "Hot Pockets ingredient list should expose pepperoni sub-ingredients");
const labelDatabaseLittleDebbieRow = labelDatabaseVisualIndex.rows.find((row) => row.product_id === "little_debbie_swiss_rolls");
assert(labelDatabaseLittleDebbieRow, "label database lane should include Little Debbie Swiss Rolls");
assert(labelDatabaseLittleDebbieRow.ingredient_items.includes("Palm and Soybean Oils with TBHQ and Citric Acid to Protect Flavor"), "Little Debbie Swiss Rolls ingredient list should expose oil preservative context");
const labelDatabaseFamily = navigator.source_family_timeline?.families?.find((row) => row.id === "label-database-text-leads");
assert(labelDatabaseFamily, "navigator should expose the label-database text leads lane");
assert.strictEqual(labelDatabaseFamily.product_count, 8, "label-database lane should expose eight products");
...
assert(labelDatabaseFamily.products.some((row) => row.product_id === "twinkies"), "navigator label-database lane should include Twinkies");
assert(labelDatabaseFamily.products.some((row) => row.product_id === "dr_pepper_original"), "navigator label-database lane should include Dr Pepper");
assert(labelDatabaseFamily.products.some((row) => row.product_id === "hot_pockets_pepperoni_pizza"), "navigator label-database lane should include Hot Pockets");
