...
assert.strictEqual(manifest.totals.capture_rows, 35, "surface map should cover the round-one capture rows");
assert.strictEqual(manifest.totals.surface_template_rows, 210, "surface map should include six surface slots per capture row");
assert.strictEqual(manifest.totals.ocr_queue_rows, 175, "surface OCR queue should include five OCR surfaces per capture row");
assert.strictEqual(manifest.totals.primary_ingredient_panel_rows, 35, "surface OCR queue should include ingredient panel rows");
assert.strictEqual(manifest.totals.primary_nutrition_panel_rows, 35, "surface OCR queue should include nutrition panel rows");
assert.strictEqual(manifest.totals.support_text_rows, 105, "surface OCR queue should include support text rows");
assert.strictEqual(manifest.totals.secondary_context_rows, 35, "surface template should keep wrapper fronts as secondary context");
assert.strictEqual(manifest.totals.ready_for_capture, 0, "surface map should not be ready without private paths");
assert.strictEqual(manifest.totals.no_private_path_supplied, 210, "every public template row should need private paths");
assert.strictEqual(manifest.totals.private_paths_supplied, 0, "public surface map should not publish private paths");
assert.strictEqual(queueRows.length, 175, "queue CSV should match OCR queue total");
assert.strictEqual(templateRows.length, 210, "template CSV should match surface total");
assert.strictEqual(auditRows.length, 210, "audit CSV should cover every surface template row");
assert.strictEqual(audit.template_rows, 210, "audit JSON should cover every surface template row");
assert.strictEqual(audit.ready_for_capture, 0, "audit should not mark public rows ready");
assert.strictEqual(audit.no_private_path_supplied, 210, "audit should require private paths");
