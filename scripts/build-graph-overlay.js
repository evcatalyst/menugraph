...
function buildEvidenceIndexes({ menus, matches, prices, dateEstimates, enrichment }) {
  ...
  for (const record of enrichmentRecords(enrichment, "sourceRouteReview")) {
    const sourceId = cleanValue(record.sourceId);
    if (!sourceId) continue;
    evidenceIndex.sourceRouteReview[sourceId] = {
      id: cleanValue(record.id || `sourcereview:${sourceId}`),
      sourceId,
      sourceKey: cleanValue(record.sourceKey),
      label: cleanValue(record.label),
      status: cleanValue(record.status),
      reviewTypes: (record.reviewTypes || []).slice(0, 8).map(cleanValue).filter(Boolean),
      priorityScore: Number(record.priorityScore || 0),
      coverage: {
        rowCount: Number(record.coverage?.rowCount || 0),
        priceCoverage: Number(record.coverage?.priceCoverage || 0),
        dishCoverage: Number(record.coverage?.dishCoverage || 0),
        ingredientCoverage: Number(record.coverage?.ingredientCoverage || 0),
        imageCoverage: Number(record.coverage?.imageCoverage || 0),
        ocrFailures: Number(record.coverage?.ocrFailures || 0),
        primaryNextAction: cleanValue(record.coverage?.primaryNextAction),
      },
      counts: {
        metadataOnlyReviewCandidates: Number(record.counts?.metadataOnlyReviewCandidates || 0),
        failureReviewCandidates: Number(record.counts?.failureReviewCandidates || 0),
        graphGaps: Number(record.counts?.graphGaps || 0),
        sampleMetadataCandidates: Number(record.counts?.sampleMetadataCandidates || 0),
        sampleFailures: Number(record.counts?.sampleFailures || 0),
        sampleGraphGaps: Number(record.counts?.sampleGraphGaps || 0),
      },
      sourceProbe: {
        routeStatus: cleanValue(record.sourceProbe?.routeStatus),
        routeBlocker: cleanValue(record.sourceProbe?.routeBlocker),
        sourceUrl: cleanValue(record.sourceProbe?.sourceUrl),
        accessMethod: cleanValue(record.sourceProbe?.accessMethod),
      },
      blockers: (record.blockers || []).slice(0, 4).map(cleanValue).filter(Boolean),
      requiredDecisions: (record.requiredDecisions || []).slice(0, 4).map(cleanValue).filter(Boolean),
      sampleCandidates: (record.sampleCandidates || []).slice(0, 4).map((candidate) => ({
        id: cleanValue(candidate.id),
        menuId: cleanValue(candidate.menuId),
        title: cleanValue(candidate.title).slice(0, 120),
        route: cleanValue(candidate.route),
        status: cleanValue(candidate.status),
      })),
      sampleFailures: (record.sampleFailures || []).slice(0, 4).map((failure) => ({
        id: cleanValue(failure.id),
        candidateId: cleanValue(failure.candidateId),
        menuId: cleanValue(failure.menuId),
        errorClass: cleanValue(failure.errorClass),
        nextAction: cleanValue(failure.nextAction),
      })),
    };
  }
...
async function buildGraphOverlay(options = {}) {
  ...
  const [
    ...
    readJson(path.join(DATA_DIR, "enrichment", "source-route-review.json"), { records: [], summary: {} }),
  ...
  evidenceIndex.summary = {
    ...
    sourceRouteReview: Object.keys(evidenceIndex.sourceRouteReview).length,
  ...
  const manifest = {
    ...
    summary: {
      ...
      enrichment: {
        ...
        sourceRouteReview: enrichmentRecords(enrichment, "sourceRouteReview").length,
