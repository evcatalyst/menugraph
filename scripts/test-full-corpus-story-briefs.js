...
const fixturePhotoManifest = path.join(runDir, "public-photo-proof.json");
...
fs.writeFileSync(fixturePhotoManifest, `${JSON.stringify({
  published_images: [{
    product_id: "sample_cookie",
    evidence_id: "sample_source",
    image_display_policy: "embed_rights_cleared",
    public_image_url: "https://example.com/sample-cookie.jpg",
  }],
}, null, 2)}\n`);
...
const result = buildFullCorpusStoryBriefs({
  ...
  publicPhotoManifestPath: fixturePhotoManifest,
  updateSiteData: false,
});

assert.strictEqual(result.summary.product_count, 1, "fixture should produce one story");
assert.strictEqual(result.summary.public_embeds, 1, "fixture should count manifest-published images");
...
assert.strictEqual(result.stories[0].public_photo_mode, "mixed_embed_and_source_receipts", "manifest image should switch photo mode");
assert(fs.readFileSync(outputMd, "utf8").includes("Sample Cookie"), "markdown should include product story");
assert(fs.readFileSync(outputCsv, "utf8").includes("mixed_embed_and_source_receipts"), "CSV should include public photo mode");
...
assert.strictEqual(defaultSummaryPayload.public_embeds, 31, "default summary should count reviewed public photo embeds");
assert.strictEqual(defaultSummaryPayload.public_photo_manifest_published_images, 31, "default summary should cite public photo manifest image count");
